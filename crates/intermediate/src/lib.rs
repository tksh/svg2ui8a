...
impl IntermediateV1 {
    ...
    let shapes: Vec<Value<'static>> = self
      ...
      .map(|shape| {
        let fill_value = match shape.fill {
          Some(Paint::Color(color)) => Value::array([Value::from(1u64), Value::from(color as u64)]),
          None => Value::from(0u64),
        };

        Value::map([
          (Value::from("path_data"), Value::from(shape.path_data.clone())),
  ...
  pub fn decode(bytes: &[u8]) -> Result<Self, DecodeError> {
    ...
    for item in shapes_array.iter() {
      ...
      let fill = match fill_value.to_u64() {
        Ok(0) => None,
        Ok(_) => {
          return Err(DecodeError::InvalidPayload("fill is not a valid value".into()));
        }
        Err(_) => {
          let array = fill_value
            .as_array()
            .map_err(|_| DecodeError::InvalidPayload("fill is not a valid value".into()))?;
          if array.len() != 2 {
            return Err(DecodeError::InvalidPayload("fill array must have two items".into()));
          }
          let tag = array[0]
            .to_u64()
            .map_err(|_| DecodeError::InvalidPayload("fill tag is not unsigned".into()))?;
          let color = array[1]
            .to_u64()
            .map_err(|_| DecodeError::InvalidPayload("fill color is not unsigned".into()))?;
          if tag != 1 {
      ...
      let opacity = shape_map
        ...
        .to_f64()
        .map_err(|_| DecodeError::InvalidPayload("opacity is not a float".into()))? as f32;

      if opacity < 0.0 || opacity > 1.0 {
        return Err(DecodeError::InvalidPayload("opacity out of range".into()));
      }

      shapes.push(Shape { path_data, fill, opacity });
    }
  ...
  pub fn to_tree(&self) -> Result<Tree, DecodeError> {
    Err(DecodeError::InvalidPayload("to_tree not implemented".into()))
  }
}
